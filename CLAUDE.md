...
This folder is the `bioacq` repository (GitHub `amiradmehr/bioacq`, private). `bioacq` is a C++17 / Qt6 Widgets real-time viewer that streams two devices through the BrainFlow C++ API: an OpenBCI Cyton (serial dongle, one ECG channel on EXG channel 1) and an EmotiBit (Bluetooth with the bioacq firmware, or WiFi: PPG green / red / IR, a heart rate derived from them, temperature, accel/gyro/mag in one IMU panel). One Connect button opens both devices. `README.md` specifies the UI behaviour, modes, networking and known limitations in detail. Read the relevant section before changing behaviour, and update it when behaviour changes.
...
The interactive GUI and `run.sh --probe [s]` (add `--no-cyton` / `--no-emotibit` to probe one device) use the real devices: the Cyton dongle, auto-detected (on this Mac `/dev/cu.usbserial-DP04W4GA`; `--list-ports` shows it without opening it), and the EmotiBit, over Bluetooth (the lab EmotiBit's firmware boots in Bluetooth mode) or at `192.168.1.12` on Wi-Fi, reached by unicast. Keep in mind:
...
- Bluetooth: macOS kills (SIGABRT, TCC) a process that touches Bluetooth when its responsible app lacks `NSBluetoothAlwaysUsageDescription`, which includes anything started from this session, Terminal or an IDE. `bluetooth_access::check` therefore returns `Unavailable` unless the process was started by launchd (`getppid () == 1`) or `BIOACQ_ALLOW_BLUETOOTH` is set, and the development binary uses Wi-Fi only. Never set `BIOACQ_ALLOW_BLUETOOTH` for a process started from here. To test Bluetooth on the real EmotiBit, package the app and launch the bundle: `open -W -n --stdout out.txt --stderr err.txt -a dist/macos/BioAcq.app --args --probe 20 --no-cyton --emotibit-link bluetooth` (the first run of a new ad-hoc build asks the user to allow Bluetooth).
