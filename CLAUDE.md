...
- Dependencies: Homebrew Qt 6 (`/opt/homebrew/opt/qt`, including Qt Serial Port and Qt Bluetooth; CI installs `qtserialport qtconnectivity`), CMake ≥ 3.21, Ninja, and BrainFlow 5.23.0 installed in `~/.local/brainflow` (patched, see below).
...
- Stages:
  ...
  4. Synthetic streaming, ECG filters, heart rate through the worker (a third session with `testPpgBpm`; the Cyton's own `cyton.hr` ring is checked too, and the synthetic board's 5 Hz sine correctly yields no rate), recording and the drop counter.
  5. Shutdown.
  6. Reconnect.
  7. Test hooks.
...
**Signal resolution.** BrainFlow row numbers are never hard-coded. `signalDefsFor` in `SignalSpec.cpp` lists ordered candidates (channel kind, preset, index), and `resolveSignals` takes the first one that exists on the board descriptor. That lets the same pipeline run on `SYNTHETIC_BOARD`, which substitutes some channels; for example, its auxiliary accelerometer stands in for the magnetometer and it has only two PPG rows. On a real board, a channel resolved through a fallback gets a `SUBST` tag. EmotiBit PPG order is `ppg_channels = [IR, red, green]` (BrainFlow's `emotibit.cpp`; asserted by the selftest). A `SignalDef` with `derivedFrom` (the heart rates, `emotibit.hr` from the three PPG channels and `cyton.hr` from the ECG) has no rows: it resolves when one of its inputs does, the worker computes it (`HeartRate::Tracker` on the channels flagged `heartRateInput`, in `Mode::Ecg` when the derived key is `cyton.hr`) and its ring holds `HeartRateRing` channels. Both devices compute their heart rate while they stream; `MainWindow::applyHeartRateSource` binds the panel to the selected one, so `plotForKey` deliberately does not map the heart-rate keys. Plots are looked up by `SignalKeys` strings (`plotForKey` also returns the IMU lane).
...
- The heart-rate source (`--heart-rate ppg|ecg`, QSettings `hr/fromEcg`, default PPG) is in `main.cpp` (`Args`, `usage ()`), `LaunchOptions` and the rail's *Heart rate from ECG* switch; `applyHeartRateSource` is the only place that binds `hrPlot_`.
...
- OS-specific user-facing text (Local Network permission on macOS, the firewall on Windows) goes through `networkPermissionHint ()` in `DeviceWorker.cpp`; the Bluetooth permission texts and the launchd check live in `BluetoothAccess.cpp`. macOS needs the static `Qt6::QDarwinBluetoothPermissionPlugin` (`qt_import_plugins` in `CMakeLists.txt`), or the discovery agent fails with `MissingPermissionsError`. Shortcut hints use `QKeySequence::NativeText` (`⌘K` / `Ctrl+K`), never literal glyphs.
