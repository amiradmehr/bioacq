...
* **Cyton (ECG).** The port field defaults to **Auto**: it prefers the last port that connected, else the first `/dev/cu.usbserial-*`. Pick or type a port to override it (a missing override is `NOT FOUND`, never a fallback to another port); the rescan button or **⌘R** rescans. BrainFlow soft-resets the board on connect, which takes a few seconds.
...
1. **Band-pass 0.5–4 Hz** per channel: one RBJ high-pass and one RBJ low-pass biquad (Q = 1/√2), designed at the nominal PPG rate and redesigned once if the timestamps show the stream runs more than 15 % off it. (A second high-pass stage made the filter ring: at 30–45 bpm its rebound in the long diastole reached half a beat's height and counted as a beat.)
